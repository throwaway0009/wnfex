...
static WnfName_t wnf_name_map[ 1163 ] =
{
    {"WNF_CMFC_FEATURE_CONFIGURATION_CHANGED", 0x280032EA3BC0875},          // win11 added manually from ntdll.dll
    {"WNF_SEB_DEV_MNF_CUSTOM_NOTIFICATION_RECEIVED", 0x41840B3EA3BCB875},   // win11 added manually from ntdll.dll
    ...
    {"WNF_AUDC_VAM_ACTIVE", 0x2821B2CA3BC6875}, // added from SndVolSSO
...
typedef union 
{
    ...
    {
        uint64_t                           Version : 4;
        uint64_t                           NameLifetime : 2;
        uint64_t                           DataScope : 4;
        uint64_t                           PermanentData : 1;
        uint64_t                           Unique : 53;
    };
...
{
public:
    static std::string state_to_name( uint64_t state )
    {
        std::string ret = "UNKNOWN_NAME";

        for( auto& n : wnf_name_map )
        {
            if( n.state_name == state )
                return n.name;
        }

        state_name_conv_t i_state;
        i_state.statename = state ^ WNF_STATE_KEY;

        switch( i_state.NameLifetime )
        {
        case WnfWellKnownStateName:
        {
            char c1 = i_state.Unique >> 21 & 0xff;
            char c2 = i_state.Unique >> 29 & 0xff;
            char c3 = i_state.Unique >> 37 & 0xff;
            char c4 = i_state.Unique >> 45 & 0xff;
            int num = i_state.Unique & 0xfffff;
        ...
        {
            int num = i_state.Unique & 0xfffff;
            ret = std::format( "WNF_PERMANENT_{:0>5X}", num );
            break;
        }
        case WnfPersistentStateName:
        {
            int num = i_state.Unique & 0xfffff;
            ret = std::format( "WNF_PERSISTENT_{:0>5X}", num );
            break;
        }
        case WnfTemporaryStateName:
        {
            int num = i_state.Unique & 0xfffff;
            ret = std::format( "WNF_TEMPORARY_{:0>5X}", num );
            break;
        }
        }
    ...
    static uint64_t name_to_state( const std::string& name )
    {
        for( auto& n : wnf_name_map )
        {
            if( n.name == name )
